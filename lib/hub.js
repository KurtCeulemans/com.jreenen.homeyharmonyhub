    ...
    websocket() {
        ...
        this.hubConnection.on('open', () => {
            ...
            this.inActivityTimerID = this.parent.homey.setInterval(() => {
                ...
                const secondsPastSinceLastMessage = ((Date.now() - this.lastMessageTime) / 1000);
                if (secondsPastSinceLastMessage >= 60) {
                    console.log(`Hub ${this.friendlyName} went offline, no hub response for ${secondsPastSinceLastMessage} seconds.`);
                    if (this.hubConnection.readyState === WebSocket.OPEN)
