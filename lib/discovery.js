...
class Discovery extends EventEmitter {
    ...
    _handleSsdpResponse(response, rinfo) {
        ...
        this._fetchSsdpDescription(location).then((desc) => {
            ...
            if (this._ssdpEnrichmentPending.has(hubInfo.ip) || this._ssdpEnrichedIps.has(hubInfo.ip))
                return;

            this._ssdpEnrichmentPending.add(hubInfo.ip);

            this._probeHubIp(hubInfo.ip, this.SSDP_ENRICHMENT_TIMEOUT).then((probeInfo) => {
                if (probeInfo.remoteId) {
                    hubInfo.remoteId = probeInfo.remoteId;
                    hubInfo.hubId = probeInfo.hubId;
                    this._ssdpEnrichedIps.add(hubInfo.ip);
                }
                if (!hubInfo.friendlyName && probeInfo.friendlyName)
                    hubInfo.friendlyName = probeInfo.friendlyName;

                this._hubManager.addHub(hubInfo);
            }).catch((err) => {
                console.log(`discovery.js: SSDP hub enrichment failed ${err}`);
            }).finally(() => {
                this._ssdpEnrichmentPending.delete(hubInfo.ip);
            });
        }).catch((err) => {
